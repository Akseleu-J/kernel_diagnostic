...
def run_live_training(steps=300, lr=3e-3, use_clip=True, clip_norm=1.0,
    ...
    # crude Adam
    def init_adam(p):
        return jax.tree_util.tree_map(lambda x: (jnp.zeros_like(x), jnp.zeros_like(x)), p)
    ...
    def adam_step(p, gr, st, t, lr_):
        ...
        flat_s, _ = jax.tree_util.tree_flatten(st)
        new_flat_p, new_flat_s = [], []
        for pp, gg, mv in zip(flat_p, flat_g, flat_s):
            np_, ns_ = upd(pp, gg, mv)
            new_flat_p.append(np_)
            new_flat_s.append(ns_)
        new_p = jax.tree_util.tree_unflatten(treedef, new_flat_p)
        new_s = jax.tree_util.tree_unflatten(treedef, new_flat_s)
        return new_p, new_s
...
def main():
    ...
    p.add_argument("--stage", type=str, default=None,
                    help="one of: A,B,C,D,B1,B2,B3,B4,B5,FULL")
    p.add_argument("--live", action="store_true")
    p.add_argument("--steps", type=int, default=300)
    p.add_argument("--lr", type=float, default=3e-3)
    p.add_argument("--no-clip", action="store_true")
    p.add_argument("--no-nan-guard", action="store_true")
    p.add_argument("--dump-dir", type=str, default="/tmp/gdn2_nonfinite_dumps")
    p.add_argument("--scenario", type=str, default=None,
                    help="reproduce chat diagnostic #2 scenarios directly: A,B,C,D")
    args = p.parse_args()
    ...
    if args.all:
        ...
        results[args.stage] = STAGE_FNS[args.stage](config)
    ...
    if args.scenario:
        ...
        run_live_training(steps=args.steps, dump_dir=args.dump_dir, **scenarios[args.scenario])
    elif args.live:
        run_live_training(steps=args.steps, lr=args.lr, use_clip=not args.no_clip,
                           use_nan_guard=not args.no_nan_guard, dump_dir=args.dump_dir)
