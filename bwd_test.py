...
def test_backward_B1(config: KernelConfig, seed=10):
    ...
    # аргумент `dv_partial` ДОЛЖЕН быть уже посчитанным вкладом от B2
    # (dav_backward_pallas's dv_new), а НЕ нулём/заглушкой. Раньше тест
    # передавал jnp.zeros_like(...)*0.0 вместо реального dv_partial --
    # из-за этого dh0_kernel закономерно не совпадал с jax.vjp
    # референсом (B1 буквально считал adjoint НЕПОЛНОЙ формулы). Это
    # была ошибка теста, не ядра -- см. чат.
    #
    # Нужно сначала посчитать Aqk/v_new (уже есть выше), потом реальный
    ...
    # (в chunk-layout (bsz,H,n_chunks,BT,D), то же, что ожидает B2/B1).
    #
    # ФИКС (был баг в предыдущей версии этого фикса): gdn2_inter_chunk_
    # combine_with_state возвращает h_pre_all/v_new_all в SCAN-НАТИВНОЙ
    # раскладке -- n_chunks является ВЕДУЩЕЙ осью (axis=0), т.е. форма
    # (n_chunks, bsz, H, BT, D), а НЕ (bsz, H, n_chunks, BT, D). Это прямо
    # задокументировано в gdn2_fwd.py (см. комментарий в
    # gdn2_pallas_forward_with_residuals: "h_pre_all/v_new_all в
    # scan-нативной раскладке... backward-кернели ожидают вторую форму"),
    # и там же лечится через jnp.moveaxis(x, 0, 2). Здесь этот moveaxis
    # был пропущен -- v_new_all передавался в dav_backward_pallas с
    # осями, где axis[2] был H=2, а не n_chunks=3, что и давало неверный
    # grid внутри пришлось Pallas-кернеля и на выходе dv_partial_from_b2
    # с n_chunks=2 вместо 3 -- отсюда
    # "scan got values with different leading axis sizes: 3, 2, 3, 3, 3, 3"
    # в B1 (do_r=3 корректный, dv_partial_from_b2=2 -- испорченный).
    _, _h_final_ws, h_pre_all_ws, v_new_all_ws = fwdmod.gdn2_inter_chunk_combine_with_state(
        Aqk, w_pseudo, u, kg, qg, gc_last, scale=1.0, config=config
    )
    h_pre_all = jnp.moveaxis(h_pre_all_ws, 0, 2)
    v_new_all = jnp.moveaxis(v_new_all_ws, 0, 2)
...
def test_backward_B3(config: KernelConfig, seed=12):
    ...
    for mode in ADVERSARIAL_MODES:
        ...
        A = fwdmod.wy_solve_pallas(Akk, config=config)
        w_pseudo, u, kg, qg, gc_last = fwdmod.recompute_wy_pallas(q, k, v, w, b, g, A, config=config)
        # ФИКС (тот же баг, что в test_backward_B1 -- см. комментарий там):
        # gdn2_inter_chunk_combine_with_state возвращает h_pre_all/v_new_all
        # в scan-нативной раскладке (n_chunks -- ведущая ось, axis=0), а
        # wy_dqkg_backward_pallas (B3) ожидает (bsz,H,n_chunks,D,D)/
        # (bsz,H,n_chunks,BT,D) -- нужен moveaxis(x, 0, 2).
        o_chunks, h_final, h_pre_all_ws, v_new_all_ws = fwdmod.gdn2_inter_chunk_combine_with_state(
            Aqk, w_pseudo, u, kg, qg, gc_last, scale=1.0, config=config
        )
        h_pre_all = jnp.moveaxis(h_pre_all_ws, 0, 2)
        v_new_all = jnp.moveaxis(v_new_all_ws, 0, 2)
